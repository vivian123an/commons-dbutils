...
public class MethodInvokingJobDetailFactoryBean implements FactoryBean, BeanNameAware, InitializingBean
{
	...
	public void afterPropertiesSet() throws Exception
	{
		try
		{
			...
			//下面是 Quartz 1.x JobDetail 的方法，Quartz 2.0 开始已经使用新的 JobBuilder
			...
			jobDetail = JobBuilder.newJob(concurrent ? MethodInvokingJob.class : StatefulMethodInvokingJob.class)
					...
					.requestRecovery(shouldRecover).build();
			...
			if (this.jobListenerNames != null) {
				for (int i = 0; i < this.jobListenerNames.length; i++) {
					//this.jobDetail.getJobDataMap()  addJobListener(this.jobListenerNames[i]);
					
					//Quartz 2.0 开始已经使用 ListenerManager
